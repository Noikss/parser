...
def get_user_info(user):
    """Получаем bio и dc_id напрямую из объекта пользователя — без доп. запросов"""
...
async def parse_channel(client, channel_username, status_msg: Message):
    ...
    try:
        ...
        await status_msg.answer(f"❌ Сессия устарела. Напиши /start и авторизуйся заново.")
        return rows
...
async def cmd_start(message: Message, state: FSMContext):
    ...
    # Сбрасываем старый клиент из памяти если есть
    ...
    # Если сессия уже есть — пробуем подключиться
...
if __name__ == "__main__":
    asyncio.run(main())    waiting_channels_file = State()
    parsing = State()
